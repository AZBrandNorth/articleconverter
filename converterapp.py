...
def analyze_code(code):
    ...
    if 'block = f"<!-- wp:paragraph -->\\n<p>{inner_html}</p>\\n<!-- /wp:paragraph -->"' in code:
        issues.append("⚠️ Found problematic paragraph block code (nested <p> tags issue)")
    
    if 'block = f\'<!-- wp:quote -->\\n<blockquote class="wp-block-quote"><p>{quote_content}</p></blockquote>\\n<!-- /wp:quote -->\'' in code:
        issues.append("⚠️ Found problematic blockquote block code (nested <p> tags issue)")
...
with st.sidebar:
    ...
    st.markdown("---")
    st.success("Made with ❤️ for BN Content Team")
